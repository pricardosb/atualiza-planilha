...
with col2:
    ...
    # Nova funcionalidade: Escolher a linha do cabeçalho
...
def ler_destino(file, header_line):
    # O Python conta linhas começando do 0, por isso subtraímos 1
    idx = header_line - 1
...
if source_file and dest_file:
    try:
        ...
        # Leitor inteligente para Origem
        ...
        if st.button("🚀 Processar e Gerar Novo Arquivo"):
            if len(final_selected) == 0:
                st.error("Nenhum registro foi marcado!")
            else:
                ...
                with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
                    # Usamos a linha do cabeçalho definida pelo usuário para salvar também
                    for sh_name, sh_df in output_dfs.items():
                        sh_df.to_excel(writer, sheet_name=sh_name, index=False, startrow=header_dest)

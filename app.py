...
# --- OPÇÃO 3: PESQUISA PARA REMIÇÃO ---
                ...
                for sheet, cfg in file_cfg.items():
                    try:
                        ...
                        if target_col and target_col in df_tmp.columns:
                            ...
                            def extrair_dados_sequenciais(row):
                                if is_sem_remuner:
                            ...
                            res_df = df_tmp.apply(extrair_dados_sequenciais, axis=1)
    ...
    if st.session_state.get('pesquisa_df') is not None:
        ...
        if not df_view.empty:
            ...
            pos_cols = [c for c in df_display_all.columns if str(c).startswith("POS_")]
            ...
            for pos_col in pos_cols:
                ...
                hdr_name = None
                if hdr_col in df_display_all.columns:
                    valid_hdrs = df_display_all[hdr_col].dropna().unique()
                    if len(valid_hdrs) > 0:
            ...
            df_render = df_display_all[cols_exibir].rename(columns=rename_map)

...
with col2:
  dest_file = st.file_uploader(
      "2. Enviar Arquivo de Destino (Seu Controle)",
      type=["xlsx", "xls", "csv"],
  )


# Função inteligente para ler Excel (.xls e .xlsx) ou CSV
  ...
  try:
    ...
    if st.button("Executar Integração"):
      ...
      if mode == "Final do arquivo":
        ...
        final_df = pd.concat(
            [dest_df.iloc[:target_line], new_data, dest_df.iloc[target_line:]],
            ignore_index=True,
        )

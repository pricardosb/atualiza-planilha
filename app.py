...
if 'arquivo_atualizado_pronto' not in st.session_state: st.session_state['arquivo_atualizado_pronto'] = None

# --- MENU LATERAL (SIDEBAR) ---
with st.sidebar:
    st.header("Navegação")
    menu_opcao = st.radio("Selecione a rotina:", [
    ...
    # Botão de download global na lateral sempre que houver arquivo pronto
    if st.session_state['arquivo_atualizado_pronto']:
        st.divider()
        st.download_button(
            label="📥 BAIXAR VERSÃO ATUALIZADA",
            data=st.session_state['arquivo_atualizado_pronto'],
            file_name="sinale_atualizado.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            use_container_width=True
        )

# --- OPÇÃO 1: SUA LÓGICA ORIGINAL COMPLETA ---
if menu_opcao == "ATUALIZAÇÃO DE DADOS - INCLUSÃO DE TRABALHO":
    ...
    if dest_file:
        if "wb_data" not in st.session_state or st.session_state.get("last_dest_name") != dest_file.name:
            ...
            st.session_state["arquivo_atualizado_pronto"] = dest_file.getvalue()
    ...
    if df_origem is not None and wb_data is not None:
        ...
        if st.button("🚀 Processar e Atualizar"):
            ...
            # Sincroniza estado para o resto do app e botão de download lateral
            st.session_state["wb_data"] = buffer.getvalue()
            st.session_state['arquivo_atualizado_pronto'] = buffer.getvalue()

            st.success("✅ Processamento concluído com sucesso! O arquivo foi atualizado na memória e está pronto para download na barra lateral.")

# --- OUTRAS OPÇÕES DO MENU ---
elif menu_opcao == "ATUALIZAÇÕES GERAIS":
    titulo_estilizado("Atualizações Gerais")
    st.info("Espaço reservado para as atualizações gerais.")

elif menu_opcao == "GERAR RELATÓRIOS":
    titulo_estilizado("Gerador de Relatórios")
    st.info("Espaço reservado para relatórios.")

elif menu_opcao == "LIMPAR ARQUIVO":
    titulo_estilizado("Limpeza de Memória")
    if st.button("🗑️ Limpar Arquivos da Memória"):
        st.session_state['source_df'] = None
        st.session_state['wb_data'] = None
        st.session_state['arquivo_atualizado_pronto'] = None
    ...
    st.info("Espaço reservado para o filtro de ativos.")
